...
async function listParticipants(postId) {
  const q = `SELECT ap.post_id, ap.user_id, ap.joined_at, u.first_name, u.last_name FROM activity_participants ap JOIN users u ON u.user_id = ap.user_id WHERE ap.post_id = $1 ORDER BY ap.joined_at`;
